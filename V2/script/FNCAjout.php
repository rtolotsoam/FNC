...
<head>
	...
	<link type="text/css" href="../css/ui.all.css?213" rel="stylesheet" />
...
<?php
	...
	if (isset($_SESSION["MSGAjout"])) echo $_SESSION['MSGAjout'] ;
	echo "</p>";
...
<?php
	...
	if (isset ($_REQUEST["txtAjoutAction"])) {
      ...
      $autre ="";
		 ...
		 * modifié
		 */
		if ($zType == "audit") $zRef = "NC_".strtoupper($zCode)."_".date("ymd");
		if ($zType == "interne") $zRef = "NC_INTERNE".strtoupper($zCode[0].$zCode[1].$zCode[2])."_".date("ymd"); // ajout 2012-07-11
		if ($zType == "client") $zRef = "NC_".strtoupper($zCode[0].$zCode[1].$zCode[2])."_".date("ymd");
		/**
		 * fin modification
		 */
		...
		if(pg_num_rows($oQueryRequete) == 0) $zRef .= ""; else $zRef .= "(".pg_num_rows($oQueryRequete).")";
      ...
      $zClientName = strtoupper(pg_escape_string($arrClient["nom_client"])) ;
		...
		if ($zType == "audit") $zClientName .= " " . $zIdClient; //strtoupper(addslashes($_REQUEST["txtClientName"])) ;
     ...
     // echo $zSqlInsertFNC ; exit;
      ...
      // ##############################################################################
		...
		*/
		...
		if (!$oInsertFNC) {
			$_SESSION["MSGAjout"] = "<font color=\"black\">Derni&egrave;re action :</font> ouverture de la fiche ayant la r&eacute;f&eacute;rence : ".$zRef."<br /><font color=\"black\">Statut :</font> erreur lors de l'ouverture de la FNC.<br />";
